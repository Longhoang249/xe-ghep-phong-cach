...
export default function BookingExperience({ routes }: Props) {
  ...
  return (
    <main>
      ...
      <footer>
        <div className="footer-brand"><span className="brand-mark">PC</span><div><strong>XE GHÉP PHONG CÁCH</strong><p>Kết nối chuyến đi tỉnh từ Hải Dương.</p></div></div>
        <div>
        ...
        <div><strong>Liên hệ</strong><a href={siteConfig.phoneHref} onClick={() => trackEvent("click_call", { placement: "home_footer" })}>{siteConfig.phoneDisplay}</a><a href={process.env.NEXT_PUBLIC_ZALO_URL || siteConfig.zaloFallbackUrl} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("click_zalo", { placement: "home_footer" })}>Zalo Phong Cách</a><Link href="/lien-he">Thông tin liên hệ</Link></div>
        <div><strong>Khám phá</strong><Link href="/blog">Blog tuyến xe</Link><Link href="/tuyen-xe">Tất cả tuyến xe</Link><Link href="/gioi-thieu">Giới thiệu</Link><Link href="/chinh-sach-dat-xe">Chính sách đặt xe</Link><Link href="/an-toan-va-doi-xe">An toàn & đội xe</Link></div>
        <p className="copyright">© 2026 Xe Ghép Phong Cách. Mức giá trên website là tham khảo và được xác nhận trước chuyến. Hình ảnh là hình minh họa được tạo bằng AI.</p>
      </footer>
