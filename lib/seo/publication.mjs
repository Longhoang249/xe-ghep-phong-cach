...
export function publicPricePresentation(fact) {
  ...
  if (typeof value !== "number") return Object.freeze({ kind: "CONTACT", text: NEUTRAL_PRICE_MESSAGE, amount: null });
  return Object.freeze({
    kind: fact.status === "ESTIMATE" ? "ESTIMATE" : "VERIFIED",
    text: null,
    amount: value,
  });
}
...
export function governRouteForPublication(route, asset, evidence) {
  if (asset.backfilledExisting) return Object.freeze({ ...route, priceFallbackPolicy: "LEGACY_FORMULA" });
