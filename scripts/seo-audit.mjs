...
for (const [key, upgrade] of Object.entries(moneyPageUpgrades)) {
  ...
  if (key === "hd-van-don") {
    ...
    if (vdShared.priceMin !== 500000 || vdPrivate.priceMin !== 1500000) {
      auditResults.criticalIssues.push("Pricing engine Vân Đồn price mismatch (expected 500k shared, 1.5m private)");
      pricingScore -= 4;
    }
  }
  if (key === "hd-cam-pha") {
  ...
  if (key === "hd-uong-bi") {
    const uongBiShared = getRoutePrice("Uông Bí", "shared");
    const uongBiPrivate = getRoutePrice("Uông Bí", "private");
    if (uongBiShared?.priceMin !== 300000 || uongBiShared?.pricingType !== "EXACT" || uongBiPrivate?.priceMin !== 600000 || uongBiPrivate?.pricingType !== "EXACT") {
      auditResults.criticalIssues.push("Pricing engine Uông Bí price mismatch (expected exact 300k shared and 600k private)");
      pricingScore -= 4;
    }
  }
}
...
// Verify unique titles and valid lengths across 42 URLs
...
if (publishedAssets.length !== 35) {
  auditResults.criticalIssues.push(`Expected 35 published assets, found ${publishedAssets.length}`);
...
if (urlInventory.totalUrls !== 42) {
  auditResults.criticalIssues.push(`URL inventory count mismatch: expected 42, got ${urlInventory.totalUrls}`);
...
| **Toàn vẹn URL & Canonical** | \`seo/url-inventory.json\` | 42 URLs toàn site | ✅ PASS (Khớp sitemap/routes) |
...
1. **Bộ quy tắc nội bộ đạt 100/100**: Toàn bộ cấu trúc thư mục, tệp nguồn giá, kiểm kê URL (42 URLs), và hàng rào phòng thủ chống trùng lặp nội dung đã được tự động hóa và vượt qua kiểm tra tĩnh.
